...
def calculate_lucky_numbers(birthdate_obj):
    ...
    lucky_set = sorted([n for n in all_nums if 1 <= n <= 49])

    # Always ensure 6 unique numbers
    seed = int(birthdate_obj.strftime("%Y%m%d"))
    rng = random.Random(seed)
    while len(lucky_set) < 6:
        next_num = rng.randint(1, 49)
        if next_num not in lucky_set:
            lucky_set.append(next_num)

    return {
        "lucky_numbers": sorted(lucky_set[:6]),
        "life_path": life_path,
...
return jsonify({
    ...
    "mindset": next((trait.split(":", 1)[1].strip() for trait in profile.get("mini_traits", []) if trait.lower().startswith("mindset")), ""),
    ...
    # 💥 New extended advice fields — make sure these are comma-separated too:
